...
$("#submit-btn").on("click", e => {
  let timeStr = $("#first-train-input")
    .val()
    .trim();
  let validTimeStr =
    timeStr.search(/^\d{2}:\d{2}$/) != -1 &&
  ...
  if (
    ...
    $(".alert-div").html(
      `<div class="alert-danger text-center">Some of your inputs are missing!!!</div>`
    );
    return false;
    ...
    $(".alert-div").html(
      `<div class="alert-success text-center">${name} Schedule added successfully!!!</div>`
    );
    return true;
  } else {
    $(".alert-div").html(
      `<div class="alert-danger text-center">Invalid First Train Time format!!!</div>`
    );
    return false;
  }
});
...
$(document).on("click", ".remove", function() {
  if (confirm(`Are you sure you want to delete this?`)) {
    $(".alert-div").html(
      `<div class="alert-info text-center"> Schedule Deleted!!!</div>`
    );
